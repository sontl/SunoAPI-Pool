...
import { cookiePool, CookiePool } from './CookiePool';
...
class SunoApi {
  ...
  private cookieId: number;

  constructor(cookie: string, cookieId: number) {
...
public async custom_generate(
  ...
  public async get_credits(): Promise<object> {
    ...
    // Update cookie credits in the database
    await cookiePool.updateCookieCredits(
      this.cookieId,
...
const newSunoApi = async () => {
  const cookie = await cookiePool.getRandomValidCookie();
  if (!cookie) {
    throw new Error("No valid cookies available");
  }
  const sunoApi = new SunoApi(cookie.value, cookie.id);
  return await sunoApi.init();
}

// Remove the old sunoApi export and replace it with this function
export async function getSunoApi(): Promise<SunoApi> {
  return await newSunoApi();
}
